...
public class OrderService
{
    ...
    public List<OrderList> findStoreOrderRetailList(int storeId, int memberId, int status)
    {
        ...
        if (99 == status)
        {
            orderList = orderDao.findByRetailMemberIdAndMemberIdAndIsRetailOrderByInsDateDesc(storeId, memberId, 1); 	// 1 表示分销订单
        }
        else
        {
            orderList = orderDao.findByRetailMemberIdAndMemberIdAndOrderStatusAndIsRetailOrderByInsDateDesc(storeId, memberId, status, 1);	// 1  表示分销订单
        }
     ...
     * @throws BusinessErrorException
     */
    public void confirmRetailOrder(int orderId, int retailMemberId, int memberId)
        throws BusinessErrorException
    {
        
        StoreOrder order = orderDao.findByOrderIdAndRetailMemberIdAndMemberId(orderId, retailMemberId, memberId);
        
        if (order == null)
            throw new BusinessErrorException(ErrorCodes.ORDER_ERROR, "此订单不存在！");
        if (Constants.ORDER_STATUS_YIFAHUO == order.getOrderStatus())
        {
            Date now = new Date();
            order.setOrderStatus(Constants.ORDER_STATUS_RECEIVED);
            order.setFinishTime(now);
     ...
     * @throws BusinessErrorException
     */
    public void cancelRetailOrder(int orderId, int retailMemberId, int memberId, OrderCancel cancel)
        throws BusinessErrorException
    {
        ...
        throws BusinessErrorException
    {
        ...
        if (couponId != 0)
        {
            ...
            logger.info("优惠券金额："+coupon.getCouponMoney());
            
            logger.info("减去优惠券后金额："+orderAmount);
            // 插入订单关联优惠劵
...
//        }
        ...
        RetailMember retailMember = retailMemberDao.findOne(storeId);
        
        
        if (retailMember == null || retailMember.getMemberState() != 1)
        {
            throw new BusinessErrorException(ErrorCodes.WRONG_STORE, "此分销商信息不存在！");
        }
        
        // 会员信息
        WechatMember member = wcmemberDao.findOne(memberId);
        ...
        // 销售店铺
        preOrder.setRetailMemberId(retailMember.getMemberId());
        preOrder.setRetailMemberName(retailMember.getWechatAliasname());

        RetailShop retailShop = retailShopDao.findByMemberId(storeId);  //店铺id 是分销商id
        if(retailShop == null){
        	throw new BusinessErrorException(ErrorCodes.WRONG_MEMBER, "分销店铺不存在！");
        }
        
        Store store = storeDao.findOne(retailMember.getStoreId());
        
        // 关联母婴店
        preOrder.setStoreId(retailMember.getStoreId());
        preOrder.setCompanyName(store.getCompanyName());
        preOrder.setRetailShopName(retailShop.getRetailShop());
//        if (StringUtils.isEmpty(retailMember.getme))
//        {
//            preOrder.setCompanyName(store.getStoreAliasName());
//        }
//        else
...
//        }
        ...
        for (OrderGoods goods : orderGoods)
        {
            ...
            // 母婴店收益
            BigDecimal goodsStoreAmountTotal = BigDecimal.ZERO;
            if(goodInfo.getStoreAmount() != null){
            	goodsStoreAmountTotal = goodInfo.getStoreAmount().multiply(new BigDecimal(goods.getGoodsNum()));
            }
...
//        }
        ...
        logger.info("分销总收益=== :"+retailAmount);
        logger.info("母婴店营收=== :"+storeAmount);
