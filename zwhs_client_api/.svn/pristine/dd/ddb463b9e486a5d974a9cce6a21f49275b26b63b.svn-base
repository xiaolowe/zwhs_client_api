...
public interface StoreGoodsDao extends JpaRepository<StoreGood, Integer>, JpaSpecificationExecutor<StoreGood> {
	...
	@Query(value="SELECT count(1) FROM store_goods WHERE store_id = ?1 "
			+ " AND goods_state = '1' AND goods_verify = '1' AND is_add_retail = '1' "
			, nativeQuery = true)
	public long countAllRetailGoodsUp(int store_id);
	
	@Query(value="SELECT count(1) FROM store_goods WHERE store_id = ?1 "
			+ " AND goods_state = '0' AND goods_verify = '1' AND is_add_retail = '1' "
			, nativeQuery = true)
	public long countAllRetailGoodsDown(int store_id);
